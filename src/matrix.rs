...
impl Matrix {
    ...
    pub fn set_row(&mut self,vec: *const f32,i:usize){
        let base = i as isize * self.row_size as isize;
        let ptr = self.mat.as_mut_ptr();
        unsafe{libc::memcpy(ptr.offset(base) as *mut libc::c_void,vec as *const libc::c_void,
                     mem::size_of::<f32>()*self.row_size)};
    }
